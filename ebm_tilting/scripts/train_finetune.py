...
def train(
    ...
    optimizer = torch.optim.AdamW(model_finetune.backbone.parameters(), lr=learning_rate, weight_decay=weight_decay)
    num_steps = num_epochs * len(dataloader)
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, num_steps)

    ema_backbone: torch.nn.Module = ema_finetune.ema_model  # type: ignore
    ema_model = Model(model_finetune.interpolant, objective, ema_backbone, clip_eta_1=False, baseline=model_baseline.backbone)
    ...
    try:
        for epoch in tqdm(range(num_epochs), desc="Epochs"):
            ...
            for images, _labels in progress_bar:
                ...
                x_t = model_finetune.interpolant.interpolate(t, z, images)

                output = model_finetune(t, x_t)

                weight = _get_loss_weight(t, model_finetune.interpolant, regressor, objective)
                ...
                grads = [
                    param.grad.detach().flatten() for param in model_finetune.backbone.parameters() if param.grad is not None
                ]
                grad_norm = torch.cat(grads).norm()

                if max_grad_norm is not None:
                    torch.nn.utils.clip_grad_norm_(model_finetune.backbone.parameters(), max_grad_norm)
                ...
                # log metrics
                eta_z = model_finetune.eta_z.detach()
...
def main():
    ...
    backbone_baseline.load_state_dict(torch.load(args.load_path, weights_only=True, map_location=device))
    
    # create finetuned model
    backbone_finetune = Unet(
        time_embedding_dim=args.time_embedding_dim,
        in_channels=1,
        out_channels=1,
        base_dim=16,
    ...
    for param in backbone_finetune.parameters():
        param.data.zero_()

    model_finetune = Model(interpolant, args.objective, backbone_finetune, clip_eta_1=False, baseline=backbone_baseline)
