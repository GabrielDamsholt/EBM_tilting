...
def parse_args():
    ...
    parser.add_argument("--load_path_baseline", type=str, required=True, help="Paths to load baseline model weights from")
    parser.add_argument("--load_path_finetune", type=str, required=True, help="Paths to load finetune model weights from")
...
def main():
    ...
    backbone_baseline.load_state_dict(torch.load(args.load_path_baseline, weights_only=True, map_location=device))
    ...
    backbone_finetune.load_state_dict(torch.load(args.load_path_finetune, weights_only=True, map_location=device))
    ...
    if args.equation == "ODE":
        integrator = ODEIntegrator(model_finetune, device)
        sample = integrator.integrate(**sample_kwargs)
    elif args.equation == "SDE":
        integrator = SDEIntegrator(model_finetune, device)
